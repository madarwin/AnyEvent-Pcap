...
sub _setup_pcap {
  ...
  if ($device =~ s/^file://) {
    ...
    if ($Net::Pcap::VERSION > 0.17) {
      # open the device
      $pcap = Net::Pcap::pcap_create($device,\$err);
      croak $err if $err;
      Net::Pcap::pcap_set_timeout($pcap,100); # 100 ms timer... can it be 0?
      Net::Pcap::pcap_set_promisc($pcap,1);
      Net::Pcap::pcap_set_snaplen($pcap,1024);
      Net::Pcap::pcap_set_immediate_mode($pcap,1); # TPACKET_V3 bug workaround
      Net::Pcap::pcap_activate($pcap);
    } else {
  ...
  my $fd = Net::Pcap::pcap_fileno($pcap);
  $self->fd($fd);
  return $pcap;
}
...
sub run {
  ...
  my $io;
  $io = AnyEvent->io(fh   => $self->fd,
		     poll => 'r',
		     cb   => sub {
		       my @args;
		       Net::Pcap::dispatch($pcap, 1,
					   sub {
					     my $header = $_[1];
					     my $packet = $_[2];
